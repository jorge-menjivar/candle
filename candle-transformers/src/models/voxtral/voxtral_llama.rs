...
impl LlamaConfig {
    pub fn into_config(self, use_flash_attn: bool) -> Config {
        Config {
            ...
            head_dim: None, // use calculated head_dim for backwards compatibility
...
impl Config {
    pub fn config_7b_v1(use_flash_attn: bool) -> Self {
        Self {
            ...
            head_dim: None, // use default calculation
            use_flash_attn,
            rms_norm_eps: 1e-6,
            rope_theta: 10_000.0,
            bos_token_id: None,
...
impl VoxtralLlama {
    ...
    pub fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        debug!("About to create embedding");
        let wte = embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("model.embed_tokens"))?;
        debug!("Embedding created");
        let lm_head = if cfg.tie_word_embeddings {
            debug!("tie word embeddings");
            Linear::from_weights(wte.embeddings().clone(), None)
        } else {
            debug!("do not tie word embeddings");
            linear(cfg.hidden_size, cfg.vocab_size, vb.pp("lm_head"))?
        };
        debug!("LM head created");
        let ln_f = RmsNorm::new(cfg.hidden_size, cfg.rms_norm_eps, vb.pp("model.norm"))?;
        debug!("LayerNorm created");
